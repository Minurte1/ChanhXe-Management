...
import { checkOtp, fetchGoogleUserInfo, loginWithGoogle, sendOtp, servicesLoginUser, servicesRegisterUser } from '../services/googleAuthService';
        ...
        <div className="auth-Form">
            <div className="auth-container">
                ...
                <div className="p-field">
                    <label>Email</label>
                    <InputText name="email" value={formData?.email} onChange={handleChange} className="p-inputtext-lg w-full" />
                </div>
                {isRegister && (
                    <>
                        {' '}
                        <div className="p-field">
                            <label>Họ tên</label>
                            <InputText name="name" value={formData?.name} onChange={handleChange} className="p-inputtext-lg w-full" />
                        </div>{' '}
                        <div className="p-field">
                            <label>Số điện thoại</label>
                            <InputText name="phone" value={formData?.phone} onChange={handleChange} className="p-inputtext-lg w-full" />
                        </div>
                    </>
                )}
                <div className="p-field">
                    <label>Mật khẩu</label>
                    <Password name="password" value={formData?.password} onChange={handleChange} className="w-full" feedback={false} toggleMask />
                </div>

                <Button label={isRegister ? 'Đăng Ký' : 'Đăng Nhập'} onClick={handleSubmit} className="p-button-lg auth-button" disabled={loading}>
                    {loading ? <i className="pi pi-spin pi-spinner" /> : isRegister ? '' : ''}
                </Button>
          ...
          >
            {isRegister ? "Đăng nhập" : "Đăng ký"}
                ...
                <Dialog header="Nhập Mã OTP" visible={otpDialog} onHide={() => setOtpDialog(false)} className="otp-dialog">
                    <div className="p-field">
                        <label>Mã OTP</label>
                        <InputText name="otp" value={formData?.otp} onChange={handleChange} className="w-full input-otp" style={{ marginLeft: '10px' }} />
                    </div>
                    <div style={{ textAlign: 'center' }}>
                        <Button label="Xác Nhận" className="p-button-sm otp-button" style={{ marginTop: '10px' }} onClick={handleCheckOtp} />
                    </div>
                </Dialog>
            </div>
        </div>
