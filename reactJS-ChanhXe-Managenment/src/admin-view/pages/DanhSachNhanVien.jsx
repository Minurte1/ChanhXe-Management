...
const DanhSachNhanVien = () => {
  ...
  const saveNhanVien = async () => {
    try {
      const { ngay_tao, ngay_cap_nhat, id_nguoi_cap_nhat, ...filteredData } =
        formData;
      ...
      showError(
        isNew ? "Lỗi khi thêm nhân viên" : "Lỗi khi cập nhật nhân viên"
      );
    }
  };
              ...
              body={(rowData) => (
                <>
                  <Button
                    ...
                    onClick={() => editNhanVien(rowData)}
                  />
                  <Button
                    icon="pi pi-trash"
                    className="p-button-rounded p-button-warning"
                    style={{ marginLeft: "8px" }}
                    onClick={() => deleteNhanVien(rowData.id)}
                  />
                </>
              )}
            />
